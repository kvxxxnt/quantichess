...
public class RatingCalculator {
  ...
  {
    double sum=0.577215;
    sum=sum+Math.log(Math.abs(x));
    for (int i=1; i<=9; i++)
    {
      sum=sum+Math.pow(x, i)/(i*fact(i));
    }
    return sum;
  }
  ...
  {
    double sum=0.577215;
    sum=sum+Math.log(Math.log(x));
    for (int i=1; i<=9; i++)
    {
      sum=sum+Math.pow(Math.log(x),i)/(i*fact(i));
    }
    return sum;
  }
  /**
  ...
  private void calculateNewRating(Rating player, List<Result> results, double elapsedRatingPeriods) throws RuntimeException {
    ...
    double a = Li( Math.pow(sigma, 2) ) + Math.log(Math.log(phi));
    ...
    if ( Math.pow(delta, 2) > Math.pow(phi, 2) + v ) {
      B = Li( Math.pow(delta, 2) - Math.pow(phi, 2) - v );
    } else {
      double k = 1;
      B = a - ( k * Math.abs(tau));
